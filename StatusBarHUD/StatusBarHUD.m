...
+ (void)showImage:(UIImage *)image text:(NSString *)text
{
    ...
    window_.frame = CGRectMake(0, - WindowH, [UIScreen mainScreen].bounds.size.width, WindowH);
    window_.hidden = NO;
    ...
    // 开启一个新的定时器
    timer_ = [NSTimer scheduledTimerWithTimeInterval:HUDStayDuration target:self selector:@selector(hide) userInfo:nil repeats:NO];
}
...
+ (void)showLoading:(NSString *)text
{
    ...
    loadingView.center = CGPointMake(button.titleLabel.frame.origin.x - 60, WindowH * 0.5);
